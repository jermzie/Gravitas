...
public:
  ...
  ConvexHull(const ConvexHull& o)
  {
          ...
          if (o.optimizedVBO)
          {
                  optimizedVBO.reset(new
  std::vector<glm::vec3>(*o.optimizedVBO)); vertices = *optimizedVBO;
          }
          else
          {
                  vertices = o.vertices;
  ...
  */
  ...
  void getHullModel(const Model &objectModel, WorldTransform objectTrans) {
    ...
    // glm::vec4 objectWorldTrans = objectTrans.GetMatrix() *
  ...
  void updateCentroid(glm::vec3 displacement) {
    ...
    glm::vec4(localCentroid, 1.0f); worldCentroid =
    glm::vec3(worldCentroidTrans);
    */
  ...
  /* ????? */
  const glm::vec3 getCentroid() const { return worldCentroid; }
