...
private:
  ...
  int find_best_sibling(int leaf_index) {
    ...
    while (!pq.empty()) {
      ...
      float new_inherited_cost =
          c.inherited_cost + direct_cost - n.box.surface_area();
  ...
  int balance_tree(int index) {
    if (nodes[index].is_leaf() || nodes[index].height < 2)
      return INDEX;
...
public:
  ...
  std::vector<int> query_node(const AABB &box) const {
    ...
    while (!stack.empty()) {
      ...
      if (!aabb_intersect(box, node.box))
        continue;
  ...
  bool ray_intersection(Ray &ray, float tmax = 500.0f) {
    ...
    while (!stack.empty()) {
      ...
      // AABB ray test
      if (!node.box.ray_intersect(ray, tmax))
        continue;
