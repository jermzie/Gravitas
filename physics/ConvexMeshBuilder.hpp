...
public:
  ...
  struct Face {
    ...
    std::uint8_t horizon_edges_on_current_iteration
        : 3; // Bit for each half edge assigned to this face, each being 0 or 1
             // depending on whether the edge belongs to horizon edge
    std::unique_ptr<std::vector<size_t>> points_on_positive_side;

    Face()
        : he(std::numeric_limits<size_t>::max()), furthest_point(0),
          furthest_point_distance(0), visibility_checked_on_iteration(0),
  ...
  size_t add_face() {
    if (disabled_faces.size()) {
      ...
      assert(!f.points_on_positive_side);

      f.furthest_point_distance = 0.0f;
  ...
  // Return pointer to conflict list vertices (re-assign to new faces)
  std::unique_ptr<std::vector<size_t>> disable_face(size_t face_index) {
    ...
    return std::move(f.points_on_positive_side);
  }
