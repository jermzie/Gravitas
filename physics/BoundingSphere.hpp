...
public:
  // Init
  void computeBoundingSphere(Model model, Transform objectTrans) {
  ...
  bool raycast(Ray &r, float &t) {

    // Equation of sphere w/ radius r, at some point P = (x, y, z), centered at
    // C = (C_x, C_y, C_z): In vector form: (C - P) * (C - P) = r^2
    ...
    // three cases -- two roots (discrim. > 0), one root (discrim. = 0), no
    // solution (discrim. < 0)
    t = (b - sqrtd) / a;
    ...
    r.t = t;
    // std::cout << "HITPOINT: " << hitPoint.x << " " << hitPoint.y << " " <<
    // hitPoint.z << "\n";
    return true;
  }
  ...
  void updateCentroid(glm::vec3 displacement) {
    ...
    // cannot do objectTrans.GetPosition() bc that returns the object COM, which
    // is diff from the worldCentroid position however, the transform matrix is
    // the same, so we just apply the same translation vector to the
    // worldCentroid
    ...
    */
    ...
    // glm::vec4 worldCentroidTrans = objectTrans.GetMatrix() *
  ...
  glm::vec3 GetCentroid() { return worldCentroid; }
};
