...
public:
  ...
  struct FaceCollision {
    ...
    FaceCollision(float distance, size_t face) : separation(distance), face_index(face) {}
  };
  ...
  struct EdgeCollsion {
    ...
    EdgeCollsion() = default;
    EdgeCollsion(float distance, std::pair<size_t, size_t> edges) : separation(distance), edge_indices(edges) {}
  };
  ...
  bool poly_poly_collision(const RigidBody &poly_A, const RigidBody &poly_B, Debugger *debug = nullptr) {
    ...
    if (debug) {
      debug->draw_line(poly_A.get_centre_of_mass(), poly_B.get_centre_of_mass(), glm::vec3(1.0f, 0.0f, 0.0f));
    }
    ...
    EdgeCollsion eab = query_edge_combos(poly_A, poly_B);
    ...
    if (contact_face_A && contact_face_B) {
      // create_face_contact(fa, poly_A, poly_B);
    } else {
      // create_edge_contact(eab, poly_A, poly_B);
    }
...
private:
  ...
  ContactManifold create_face_contact(const FaceCollision &fab, const RigidBody &poly_A, const RigidBody &poly_B) {
    // NEED TO BIAS ONE POLY OVER THE OTHER
    ...
    ConvexMesh::Face reference_face = mesh_A.faces[fab.face_index];
    ConvexMesh::Face incident_face = find_incident_face(reference_face.plane.normal, poly_B);
  ...
  ContactManifold create_edge_contact(EdgeCollsion eab, const RigidBody &poly_A, const RigidBody &poly_B) {
    ...
    */
    ...
    if (std::abs(denom) < 1e-6f) {
      ...
      float s = (a_b * b_w0 - b * a_w0) / denom;
      float t = (a * b_w0 - a_b * a_w0) / denom;
      ...
      */
      ...
      if (distance > 1e-6f) {
        out.normal = contact_vector / distance;
      } else {
        // Edges are essentially touching - use cross product
        out.normal = glm::normalize(glm::cross(edge_A, edge_B));
      }
  ...
  EdgeCollsion query_edge_combos(const RigidBody &poly_A, const RigidBody &poly_B) {
    glm::mat4 modelB = poly_B.get_physics_matrix();
    glm::mat4 modelA = poly_A.get_physics_matrix();
    ...
    glm::vec3 centroid_A = glm::vec3(transform * glm::vec4(poly_A.get_geometric_centriod(), 1.0f));
    ...
    for (size_t i = 0; i < edge_count_A; i++) {
      ...
      // transform A's vertices into B's local space
      glm::vec3 p1 = glm::vec3(transform * glm::vec4(vertices_A[he_indices_A[0]], 1.0f));
      glm::vec3 q1 = glm::vec3(transform * glm::vec4(vertices_A[he_indices_A[1]], 1.0f));
      ...
      glm::vec3 local_norm_B = mesh_A.faces[mesh_A.half_edges[half_edge_A.twin].face].plane.normal;
      ...
      for (size_t j = 0; j < edge_count_B; j++) {
        ...
        glm::vec3 normal_C = glm::normalize(mesh_B.faces[half_edge_B.face].plane.normal);
        glm::vec3 normal_D = glm::normalize(mesh_B.faces[mesh_B.half_edges[half_edge_B.twin].face].plane.normal);
    ...
    return EdgeCollsion(max_distance, max_index);
  }
