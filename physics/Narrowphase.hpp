...
public:
  ...
  struct FaceColInfo {
    ...
    FaceColInfo(float distance, size_t face)
        : separation(distance), faceIdx(face) {}
  };
  ...
  struct EdgeColInfo {
    ...
    EdgeColInfo(float distance, std::pair<size_t, size_t> edges)
        : separation(distance), edgeIdx(edges) {}
  };
  ...
  bool SATPolyPoly(const ConvexHull &polyA, const ConvexHull &polyB) {
    ...
    printf("polyA centroid");
    print_vec(polyA.getCentroid());
    printf("polyB centroid");
    print_vec(polyB.getCentroid());
    ...
    glm::mat4 modelA =
        glm::inverse(modelB) * polyA.getWorldTransform().GetMatrix();
  ...
  EdgeColInfo queryEdgeCombos(const ConvexHull &polyA,
                              const ConvexHull &polyB) {
    ...
    glm::vec3 centroidA =
        glm::vec3(trans * glm::vec4(polyA.getCentroid(), 1.0f));
    ...
    for (size_t i = 0; i < edgeCountA; i++) {
      ...
      glm::vec3 bLocal =
          polyA.mesh.faces[polyA.mesh.halfEdges[halfEdgeA.twin].face].P.normal;
      ...
      for (size_t j = 0; j < edgeCountB; j++) {
        ...
        glm::vec3 cNorm =
            glm::normalize(polyB.mesh.faces[halfEdgeB.face].P.normal);
        glm::vec3 dNorm = glm::normalize(
            polyB.mesh.faces[polyB.mesh.halfEdges[halfEdgeB.twin].face]
                .P.normal);
        ...
        if (isMinkowskiFace(aNorm, bNorm, -cNorm, -dNorm)) {
          ...
          if (crossLength >
              0.005f * glm::sqrt(glm::length2(edgeA) * glm::length2(edgeB))) {
  ...
  bool isMinkowskiFace(const glm::vec3 &a, const glm::vec3 &b,
