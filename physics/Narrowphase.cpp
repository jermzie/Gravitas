...
bool Narrowphase::poly_plane_collision(ContactManifold &out,
  ...
  if (debug) {
    ...
    for (int i = 0; i < out.num_points; ++i) {
      ...
      // CLOGI("CONTACT %d PENETRATION: %.2f", i + 1,
      // out.points[i].penetration);
    }
  }
...
bool Narrowphase::poly_poly_collision(ContactManifold &out,
  ...
  EdgeColInfo eab =
      query_edge_combos(poly_A, poly_B, matrix_A, matrix_B, A_to_B, normal_A);
  ...
  if (contact_face_A || contact_face_B) {
    ...
    CLOGI("NUM CONTACTS: %lu", out.num_points);
    ...
    if (debug) {
      ...
      for (int i = 0; i < out.num_points; i++) {
        ...
        debug->draw_vertex(out.points[i].pos, glm::vec4(1, 0, 0, 1));
        points.push_back(out.points[i].pos);
        CLOGI("CONTACT %d PENETRATION: %.2f", i + 1, out.points[i].pen_depth);
      }
      debug->draw_unordered_polygon(points, glm::vec4(1, 0, 0, 1));
    }
    ...
    if (debug) {
      ...
      auto e1 = mesh_A.get_half_edge_vertices(
          mesh_A.half_edges[eab.edge_indices.first]);
      auto e2 = mesh_B.get_half_edge_vertices(
          mesh_B.half_edges[eab.edge_indices.second]);
      glm::vec3 v11 =
          glm::vec3(transform_A * glm::vec4(mesh_A.vertices[e1[0]], 1.0f));
      glm::vec3 v12 =
          glm::vec3(transform_A * glm::vec4(mesh_A.vertices[e1[1]], 1.0f));
      glm::vec3 v21 =
          glm::vec3(transform_B * glm::vec4(mesh_B.vertices[e2[0]], 1.0f));
      glm::vec3 v22 =
          glm::vec3(transform_B * glm::vec4(mesh_B.vertices[e2[1]], 1.0f));

      debug->draw_line(v11, v12, glm::vec4(1, 0, 0, 1));
      debug->draw_line(v21, v22, glm::vec4(1, 0, 0, 1));
      debug->draw_vertex(out.points[0].pos, glm::vec4(1, 0, 0, 1));

      CLOGI("PENETRATION: %.2f", out.points[0].pen_depth);
    }
  }
...
FaceColInfo Narrowphase::query_face_normals(const RigidBody &poly_A,
  ...
  glm::mat3 norm_matrix = get_normal_matrix(transform);
  ...
  for (size_t i = 0; i < face_count; i++) {
    ...
    plane.normal = norm_matrix * plane.normal;
  ...
  return FaceColInfo(max_separation, max_index);
}
...
EdgeColInfo Narrowphase::query_edge_combos(const RigidBody &poly_A,
  ...
  // glm::vec3 centroid_A = glm::vec3(transform *
  // glm::vec4(poly_A.get_local_geometric_centroid(), 1.0f));
  ...
  for (size_t i = 0; i < edge_count_A; i++) {
    ...
    glm::vec3 p1 =
        glm::vec3(A_to_B * glm::vec4(vertices_A[he_vertices_A[0]], 1.0f));
    glm::vec3 q1 =
        glm::vec3(A_to_B * glm::vec4(vertices_A[he_vertices_A[1]], 1.0f));
    ...
    for (size_t j = 0; j < edge_count_B; j++) {
      ...
      if (is_minkowski_face(normal_A, normal_B, -normal_C, -normal_D)) {
        ...
        if (cross_len >
            0.005f * glm::sqrt(glm::length2(edge_A) * glm::length2(edge_B))) {
...
FaceContact select_reference_face(const RigidBody &a, const RigidBody &b,
...
ContactManifold Narrowphase::create_face_contact(const FaceColInfo &fa,
                                                 const FaceColInfo &fb,
                                                 const RigidBody &poly_A,
                                                 const RigidBody &poly_B) {
  ...
  ContactManifold out;
  out.num_points = 0;
  out.max_pen_depth = -std::numeric_limits<float>::max();
  ...
  if (is_reference_A) {
    ...
    reference_face = poly_A.get_mesh().faces[fa.face_index];
    ...
    glm::mat3 reference_norm_matrix =
        reference_poly->transform.get_normal_matrix();
    glm::vec3 norm =
        glm::normalize(reference_norm_matrix * reference_face.plane.normal);
    incident_face = find_incident_face(norm, poly_B);
  } else {
    reference_poly = &poly_B;
    reference_mesh = poly_B.get_mesh();
    reference_face = poly_B.get_mesh().faces[fb.face_index];
    reference_transform = reference_poly->get_physics_matrix();
    ...
    glm::vec3 norm =
        glm::normalize(reference_norm_matrix * reference_face.plane.normal);
    incident_face = find_incident_face(norm, poly_A);
  }
  ...
  for (size_t index : incident_vertices) {
    incident_polygon.push_back(glm::vec3(
        incident_transform * glm::vec4(incident_mesh.vertices[index], 1.0f)));
  }
  ...
  reference_plane.normal =
      glm::normalize(reference_norm_matrix * reference_plane.normal);
  reference_plane.point =
      glm::vec3(reference_transform * glm::vec4(reference_plane.point, 1.0f));
  reference_plane.distance =
      -glm::dot(reference_plane.normal, reference_plane.point);
  ...
  for (size_t index : reference_vertices) {
    ref_face_center += glm::vec3(
        reference_transform * glm::vec4(reference_mesh.vertices[index], 1.0f));
  }
  ref_face_center /= reference_vertices.size();

  auto reference_half_edges =
      reference_mesh.get_face_half_edges(reference_face);
  for (size_t index : reference_half_edges) {
    ...
    glm::vec3 e0 =
        glm::vec3(reference_transform *
                  glm::vec4(reference_mesh.vertices[he_vertices[0]], 1.0f));
    glm::vec3 e1 =
        glm::vec3(reference_transform *
                  glm::vec4(reference_mesh.vertices[he_vertices[1]], 1.0f));
  ...
  for (const auto &point : incident_polygon) {
    ...
    if (separation <= 0.0f) {
      ...
      if (out.max_pen_depth < c.pen_depth) {
        // if (penetration < out.max_penetration) {
        out.max_pen_depth = c.pen_depth;
      }
    }
  }

  std::vector<ContactPoint> reduced_contacts =
      reduce_contact_manifold(all_contacts, reference_plane.normal);

  out.norm = reference_plane.normal;
...
ContactManifold Narrowphase::create_edge_contact(EdgeColInfo eab,
                                                 const RigidBody &poly_A,
                                                 const RigidBody &poly_B) {

  ContactManifold out;
  out.num_points = 1;
  out.norm = glm::vec3(0.0f);
  ...
  glm::vec3 p1 = glm::vec3(transform_A *
                           glm::vec4(mesh_A.vertices[he_indices_A[0]], 1.0f));
  glm::vec3 q1 = glm::vec3(transform_A *
                           glm::vec4(mesh_A.vertices[he_indices_A[1]], 1.0f));
  glm::vec3 p2 = glm::vec3(transform_B *
                           glm::vec4(mesh_B.vertices[he_indices_B[0]], 1.0f));
  glm::vec3 q2 = glm::vec3(transform_B *
                           glm::vec4(mesh_B.vertices[he_indices_B[1]], 1.0f));
  ...
  */
  ...
  if (std::abs(denom) < epsilon) {
    ...
    */
    ...
    out.points[0].pen_depth = length;
    out.max_pen_depth = length;
  }

  out.points[0].norm = out.norm;
  out.points[0].id;
...
ContactManifold Narrowphase::create_plane_contact(const RigidBody &poly,
  ...
  for (const auto &local : local_vertices) {
    ...
    if (separation <= 0.0f) {
      ...
      if (c.pen_depth > out.max_pen_depth) {
        out.max_pen_depth = c.pen_depth;
      }
    }
  }
  ...
  std::vector<ContactPoint> reduced =
      reduce_contact_manifold(candidates, plane.normal);
...
ConvexMesh::Face
Narrowphase::find_incident_face(const glm::vec3 &ref_norm_world,
                                const RigidBody &poly) {
  ...
  // transforms)
  glm::mat3 inv_rot_matrix =
      glm::transpose(glm::mat3(poly.get_physics_matrix()));
  ...
  for (const auto &face : poly.get_mesh().faces) {
    float dot = glm::dot(ref_norm_local, face.plane.normal);
    if (dot < min_dot) {
      min_dot = dot;
      incident_face = face;
    }
...
Narrowphase::reduce_contact_manifold(std::vector<ContactPoint> contacts,
                                     glm::vec3 normal) {
  if (contacts.size() <= 4) {
    return contacts;
  }
  ...
  int deepest_index = 0;
  float max_penetration = contacts[0].pen_depth;

  for (int i = 1; i < contacts.size(); i++) {
    if (contacts[i].pen_depth > max_penetration) {
      max_penetration = contacts[i].pen_depth;
      deepest_index = i;
    }
  }
  ...
  for (int i = 0; i < contacts.size(); ++i) {
    ...
    float distance =
        glm::distance2(contacts[i].pos, contacts[deepest_index].pos);
  ...
  for (int i = 0; i < contacts.size(); ++i) {
    ...
    float area = get_signed_triangle_area(contacts[deepest_index].pos,
                                          contacts[max_index].pos,
                                          contacts[i].pos, normal);
  ...
  for (int i = 0; i < contacts.size(); ++i) {
    ...
    float area0 = get_signed_triangle_area(contacts[deepest_index].pos,
                                           contacts[max_index].pos,
                                           contacts[i].pos, normal);

    // Edge 1: farthest -> third
    float area1 = get_signed_triangle_area(contacts[max_index].pos,
                                           contacts[third_index].pos,
                                           contacts[i].pos, normal);

    // Edge 2: third -> deepest
    float area2 = get_signed_triangle_area(contacts[third_index].pos,
                                           contacts[deepest_index].pos,
                                           contacts[i].pos, normal);
