...
public:
  ...
  Model(string const &model_name, bool gamma = false)
  ...
  void load_model(string const &model_name) {
    string const &path =
        std::string(PROJECT_SOURCE_DIR) + "/resources/" + model_name;
  ...
  vector<glm::vec3> get_vertex_data() const {

    vector<glm::vec3> vertex_data;
    for (auto &mesh : meshes) {
      for (auto &vertex : mesh.vertices) {
        vertex_data.push_back(vertex.position);
      }
    }
...
private:
  ...
  Mesh process_mesh(aiMesh *mesh, const aiScene *scene) {
    ...
    // 1. diffuse maps
    vector<Texture> diffuseMaps = load_material_textures(
        material, aiTextureType_DIFFUSE, "texture_diffuse");
    textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
    // 2. specular maps
    vector<Texture> specularMaps = load_material_textures(
...
// retrieve texture
inline unsigned int texture_from_file(const char *path, const string &directory,
                                      bool gamma) {
  ...
  unsigned char *data =
      stbi_load(file_name.c_str(), &width, &height, &nrComponents, 0);
  if (data) {
    GLenum format;
    if (nrComponents == 1)
      format = GL_RED;
