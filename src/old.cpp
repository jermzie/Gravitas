...
void App::InitScene() {
    ...
    RigidBody ball(ballModel, 5.0, glm::vec3(0.0, 2.0, -3.0), glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 0.0, 0.0));

    scene.addRigidBody(cube);
    scene.addRigidBody(cyl);
    //scene.addRigidBody(ball);
...
void App::Update() {
    ...
    if (leftMouseButton.isDown) {
        ...
        // unproject mouse coords as ray in world space
        ...
        if (isDragging && selectedObjectId != -1) {
            ...
            float t;
            if (picker.RayPlaneIntersection(ray, dragPlaneNormal, dragPlanePoint, t)) {

                // point of intersection
                glm::vec3 newPos = ray.origin + ray.direction * t;

                //std::cout << "Ray intersection: " << newPos.x << " " << newPos.y << " " << newPos.z << "\n";

                // Move the selected sphere to the new position

                glm::vec3 currentPos = initialObjectPos;
                glm::vec3 displacement = newPos - currentPos;
            ...
            for (int i = 0; i < scene.rigidBodies.size(); ++i) {
                ...
                if (scene.rigidBodies[i].collided(ray)) {
                    ...
                    dragPlaneNormal = -camera.Front;
                    dragPlanePoint = initialObjectPos;
...
void App::OnFramebufferSize(int width, int height) { glViewport(0, 0, width, height); }
